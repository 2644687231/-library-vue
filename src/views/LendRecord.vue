<template>
  <div class="home" style="padding: 10px">
    ...
    <div style="margin: 10px 0;">
      <el-form :model="paramsForm" inline="true" size="small">
        ...
        <el-form-item label="读者编号">
          <el-input v-model="paramsForm.userid" placeholder="请输入读者编号" clearable>
            <template #prefix>
              <el-icon class="el-input__icon">
                <search/>
              </el-icon>
    ...
    <el-table :data="tableData" stripe border="true" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="lendDate" label="借阅时间" sortable/>
      <el-table-column prop="retDate" label="归还时间" sortable/>
...
export default defineComponent({
  ...
  methods: {
    ...
    deleteBatch() {
      ...
      request.post("/Alluserlog/deleteRecords", this.forms).then(res => {
        if (res.code === 0) {
    ...
    load() {
      request.get("Alluserlog/select/" + this.user.userid, {
        params: {
    ...
    save(isbn) {
      if (this.form.isbn) {
        request.post("/Alluserlog" + isbn, this.form).then(res => {
          console.log(res)
          if (res.code == 0) {
            ElMessage({
              message: '新增成功',
        ...
        request.put("/Alluserlog/" + isbn, this.form).then(res => {
          console.log(res)
          if (res.code == 0) {
            ElMessage({
              message: '更新成功',
    ...
    handleDelete(row) {
      ...
      request.post("Alluserlog/deleteRecord", form3).then(res => {
        console.log(res)
        if (res.code == 0) {
